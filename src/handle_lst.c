...
/*   Updated: 2025/03/21 14:22:58 by ancarol9         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_lst_split(t_stack_node **lst_node, char *nbrs)
{
	...
	while (nbr_arr[i])
	{
		if (!is_digit(nbr_arr[i]))
			exit_program_failure(lst_node, nbr_arr, "Error");
...
void	create_lst(t_stack_node **lst, char **nbr)
{
	...
	{
		if (!is_digit(nbr[i]))
			exit_program_failure(lst, NULL, "Error");
		nb = ft_atol(nbr[i]);
		if (nb > INT_MAX || nb < INT_MIN)
			exit_program_failure(lst, NULL, "Error");
		node = create_node(nb, i);
		if (!node)
			exit_program_failure(lst, NULL, NULL);
